...
func (c *Client) DeleteWorkItem(ctx context.Context, id int, destroy bool) (map[string]interface{}, error) {
	path := fmt.Sprintf("%s/_apis/wit/workitems/%d", c.project, id)
	params := url.Values{}
	params.Set("api-version", defaultAPIVersion)
	if destroy {
		params.Set("destroy", "true")
	}
	respBody, err := c.do(ctx, http.MethodDelete, path, params, nil, "")
	if err != nil {
		return nil, err
	}
	if len(respBody) == 0 {
...
func (c *Client) CreatePullRequest(ctx context.Context, repository string, req CreatePullRequestRequest) (GitPullRequest, error) {
	if strings.TrimSpace(repository) == "" {
		return GitPullRequest{}, errs.New("invalid_args", "repository is required", nil)
	}
	path := fmt.Sprintf("%s/_apis/git/repositories/%s/pullrequests", c.project, url.PathEscape(repository))
	params := url.Values{}
	params.Set("api-version", defaultAPIVersion)
...
func (c *Client) UpdatePullRequest(ctx context.Context, repository string, pullRequestID int, req UpdatePullRequestRequest) (GitPullRequest, error) {
	if strings.TrimSpace(repository) == "" {
		return GitPullRequest{}, errs.New("invalid_args", "repository is required", nil)
	}
	if pullRequestID <= 0 {
	...
	respBody, err := c.do(ctx, http.MethodPatch, path, params, body, "application/json")
	if err != nil {
		return GitPullRequest{}, err
	}
	var pr GitPullRequest
